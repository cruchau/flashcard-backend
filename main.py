...
def get_cards():
    ...
    rows = c.fetchall()
    return [Flashcard(id=row[0], course=row[1], chapter=row[2], notion=row[3], question=row[4], answer=row[5], score=row[6]) for row in rows]

@app.post("/api/cards", response_model=Flashcard)
def create_card(card: Flashcard):
    c.execute("INSERT INTO flashcards (course, chapter, notion, question, answer, score) VALUES (?, ?, ?, ?, ?, ?)",
              (card.course, card.chapter, card.notion, card.question, card.answer, card.score))
    conn.commit()
    card.id = c.lastrowid
    return card
...
def update_card(card_id: int, card: Flashcard):
    c.execute("UPDATE flashcards SET course = ?, chapter = ?, notion = ?, question = ?, answer = ?, score = ? WHERE id = ?",
              (card.course, card.chapter, card.notion, card.question, card.answer, card.score, card_id))
    conn.commit()
    return card
...
def upload_csv(file: UploadFile = File(...)):
    ...
    for row in reader:
        c.execute("INSERT INTO flashcards (course, chapter, notion, question, answer, score) VALUES (?, ?, ?, ?, ?, ?)",
                  (row['Course'], row['Chapter'], row['Notion'], row['Question'], row['Answer'], int(row.get('Score', 0))))
    conn.commit()
...
def get_card_to_review():
    ...
    return Flashcard(id=selected[0], course=selected[1], chapter=selected[2], notion=selected[3], question=selected[4], answer=selected[5], score=selected[6])
